...
%%-----------------------------------------------------------------------------
%% ---------------------------------------------------------------------------
%% GlowVM shim. Based on AtomVM's estdlib maps.erl (Apache-2.0, copyright above
%% — see THIRD_PARTY_LICENSES), extended at the bottom of this file with the
%% functions Gleam needs that AtomVM does not ship.
%%
%% gleam/dict's take/2 and drop/2 are `@external(erlang, "maps", "with")` and
%% `"without"`, and gleam/list's group/2 goes through maps:update_with/4 — none
%% of which exist in AtomVM. gleam/set's take, intersection, is_subset,
%% is_disjoint and symmetric_difference all funnel through dict.take/drop, so
%% they fail for the same reason.
%%
%% This module also joins the *host* code path and shadows OTP's maps, so the
%% additions below cover OTP's surface too, not just what Gleam calls. On the
%% host, get/2, put/3, to_list/1, from_list/1, remove/2 and friends are BIFs
%% that the loader binds directly, so a caller never reaches the bodies here;
%% on AtomVM they are real functions or NIFs. The extensions are written using
%% only those four primitives precisely because that set behaves identically on
%% both VMs — in particular nothing here depends on the iterator representation.
%% ---------------------------------------------------------------------------
-module(maps).
...
-export([
    ...
    remove/2,
    update/3,
    %% GlowVM additions — see the bottom of this file.
    with/2,
    without/2,
    update_with/3,
    update_with/4,
...
%% GlowVM: on AtomVM this is a NIF, bound at every call site by the import
%% table, so the body below is dead code there. On the host there is no such
%% NIF and the body does run, so it delegates to OTP's own iteration primitive
%% rather than raising nif_error — otherwise every host caller of maps:fold/3,
%% maps:to_list/1 and friends dies, taking kernel_sup down at boot with it.
%% The iterator shape is the same on both VMs ([Path | Map]), so nothing else
%% in this module has to change.
next({Key, Value, Iterator}) ->
    {Key, Value, Iterator};
next(none) ->
    none;
next([Path | Map] = Iterator) when (is_integer(Path) orelse is_list(Path)), is_map(Map) ->
    try erts_internal:map_next(Path, Map, iterator) of
        Result -> Result
    catch
        error:badarg -> error(badarg, [Iterator]);
        error:undef -> error(badarg, [Iterator])
    end;
next(Iterator) ->
    error(badarg, [Iterator]).
...
%% GlowVM: NIF on AtomVM (dead body there), plain Erlang on the host.
from_keys(List, Value) when is_list(List) ->
    from_keys_loop(List, Value, #{});
from_keys(List, _Value) ->
    error(badarg, [List]).
...
%% GlowVM additions
%%
%% Everything below is expressed in terms of to_list/1, from_list/1, put/3 and
%% remove/2 only, so it behaves the same whether those resolve to AtomVM's
%% implementations or to OTP's BIFs.
%%-----------------------------------------------------------------------------

%%-----------------------------------------------------------------------------
%% @param   Ks   the keys to retain
%% @param   Map  the map
%% @returns a map containing only the associations whose key is in `Ks'
%% @doc Returns a new map containing only the keys in `Ks' that are present in
%% `Map'. Keys in `Ks' that are absent from `Map' are ignored.
%% @end
%%-----------------------------------------------------------------------------
-spec with(Ks :: [Key], Map :: #{Key => Value}) -> #{Key => Value}.
with(Ks, Map) when is_list(Ks) andalso is_map(Map) ->
    with_loop(Ks, Map, #{});
with(Ks, Map) when is_list(Ks) ->
    error({badmap, Map});
with(_Ks, _Map) ->
    error(badarg).

%% @private
with_loop([], _Map, Accum) ->
    Accum;
with_loop([Key | Rest], Map, Accum) ->
    case maps:find(Key, Map) of
        {ok, Value} -> with_loop(Rest, Map, maps:put(Key, Value, Accum));
        error -> with_loop(Rest, Map, Accum)
    end;
with_loop(_Ks, _Map, _Accum) ->
    error(badarg).

%%-----------------------------------------------------------------------------
%% @param   Ks   the keys to drop
%% @param   Map  the map
%% @returns `Map' without any association whose key is in `Ks'
%% @doc Returns a new map with the keys in `Ks' removed. Keys in `Ks' that are
%% absent from `Map' are ignored.
%% @end
%%-----------------------------------------------------------------------------
-spec without(Ks :: [Key], Map :: #{Key => Value}) -> #{Key => Value}.
without(Ks, Map) when is_list(Ks) andalso is_map(Map) ->
    without_loop(Ks, Map);
without(Ks, Map) when is_list(Ks) ->
    error({badmap, Map});
without(_Ks, _Map) ->
    error(badarg).
...
without_loop([], Map) ->
    Map;
without_loop([Key | Rest], Map) ->
    without_loop(Rest, maps:remove(Key, Map));
without_loop(_Ks, _Map) ->
    error(badarg).

%%-----------------------------------------------------------------------------
%% @param   Key  the key to update
%% @param   Fun  applied to the existing value
%% @param   Map  the map
%% @returns `Map' with `Key' updated to `Fun(OldValue)'
%% @doc Raises `{badkey, Key}' if `Key' is not present in `Map'.
%% @end
%%-----------------------------------------------------------------------------
-spec update_with(Key, Fun :: fun((Value) -> Value), Map :: #{Key => Value}) ->
    #{Key => Value}.
...
update_with(_Key, Fun, Map) when is_function(Fun, 1) ->
    error({badmap, Map});
...
%% @param   Key   the key to update
%% @param   Fun   applied to the existing value
%% @param   Init  inserted when `Key' is absent
%% @param   Map   the map
%% @returns `Map' with `Key' set to `Fun(OldValue)', or to `Init' if absent
%% @end
%%-----------------------------------------------------------------------------
-spec update_with(Key, Fun :: fun((Value) -> Value), Init :: Value, Map :: #{Key => Value}) ->
    #{Key => Value}.
...
update_with(_Key, Fun, _Init, Map) when is_function(Fun, 1) ->
    error({badmap, Map});
update_with(_Key, _Fun, _Init, _Map) ->
    error(badarg).

%%-----------------------------------------------------------------------------
%% @param   Fun  returns `true', `false', or `{true, NewValue}'
%% @param   Map  the map
%% @returns a map of the kept (and possibly rewritten) associations
%% @end
%%-----------------------------------------------------------------------------
-spec filtermap(Fun :: fun((Key, Value) -> boolean() | {true, Value}), Map :: #{Key => Value}) ->
    #{Key => Value}.
filtermap(Fun, Map) when is_function(Fun, 2) andalso is_map(Map) ->
    filtermap_loop(Fun, maps:to_list(Map), #{});
...
%% @private
filtermap_loop(_Fun, [], Accum) ->
    Accum;
filtermap_loop(Fun, [{Key, Value} | Rest], Accum) ->
    case Fun(Key, Value) of
        true -> filtermap_loop(Fun, Rest, maps:put(Key, Value, Accum));
        {true, NewValue} -> filtermap_loop(Fun, Rest, maps:put(Key, NewValue, Accum));
        false -> filtermap_loop(Fun, Rest, Accum)
    end.

%%-----------------------------------------------------------------------------
%% @param   Key  the key to take
%% @param   Map  the map
%% @returns `{Value, MapWithoutKey}', or `error' if `Key' is absent
%% @end
%%-----------------------------------------------------------------------------
-spec take(Key, Map :: #{Key => Value}) -> {Value, #{Key => Value}} | error.
...
%%-----------------------------------------------------------------------------
%% @returns the associations of `Map1' whose keys also appear in `Map2',
%%          taking values from `Map2'
%% @end
%%-----------------------------------------------------------------------------
-spec intersect(Map1 :: #{Key => Value}, Map2 :: #{Key => Value}) -> #{Key => Value}.
intersect(Map1, Map2) when is_map(Map1) andalso is_map(Map2) ->
    intersect_with(fun(_Key, _V1, V2) -> V2 end, Map1, Map2);
intersect(Map1, Map2) when is_map(Map1) ->
    error({badmap, Map2});
intersect(Map1, _Map2) ->
    error({badmap, Map1}).

%%-----------------------------------------------------------------------------
%% @returns the associations present in both maps, with values combined by
...
intersect_with(Combiner, Map1, Map2) when
    is_function(Combiner, 3) andalso is_map(Map1) andalso is_map(Map2)
->
    intersect_loop(Combiner, maps:to_list(Map1), Map2, #{});
intersect_with(Combiner, Map1, Map2) when is_function(Combiner, 3) andalso is_map(Map1) ->
    error({badmap, Map2});
intersect_with(Combiner, _Map1, _Map2) when is_function(Combiner, 3) ->
    error(badarg);
intersect_with(_Combiner, _Map1, _Map2) ->
    error(badarg).

%% @private
intersect_loop(_Combiner, [], _Map2, Accum) ->
    Accum;
intersect_loop(Combiner, [{Key, Value1} | Rest], Map2, Accum) ->
    case maps:find(Key, Map2) of
        {ok, Value2} ->
            intersect_loop(Combiner, Rest, Map2, maps:put(Key, Combiner(Key, Value1, Value2), Accum));
        error ->
            intersect_loop(Combiner, Rest, Map2, Accum)
    end.

%%-----------------------------------------------------------------------------
%% @equiv groups_from_list(KeyFun, fun(Elem) -> Elem end, List)
%% @end
%%-----------------------------------------------------------------------------
-spec groups_from_list(KeyFun :: fun((Elem) -> Key), List :: [Elem]) -> #{Key => [Elem]}.
groups_from_list(KeyFun, List) when is_function(KeyFun, 1) andalso is_list(List) ->
    groups_from_list(KeyFun, fun(Elem) -> Elem end, List);
groups_from_list(_KeyFun, _List) ->
    error(badarg).

%%-----------------------------------------------------------------------------
%% @param   KeyFun    computes the group key for an element
%% @param   ValueFun  computes the value stored for an element
%% @param   List      the elements to group
%% @returns a map from key to the list of values in input order
%% @end
%%-----------------------------------------------------------------------------
...
groups_from_list(KeyFun, ValueFun, List) when
    is_function(KeyFun, 1) andalso is_function(ValueFun, 1) andalso is_list(List)
->
    %% Build each group reversed, then reverse once at the end, so the result
    %% preserves input order without quadratic appends.
    Grouped = groups_loop(KeyFun, ValueFun, List, #{}),
    iterate_from_list(
        [{Key, lists:reverse(Reversed)} || {Key, Reversed} <- maps:to_list(Grouped)], #{}
    );
groups_from_list(_KeyFun, _ValueFun, _List) ->
    error(badarg).

%% @private
groups_loop(_KeyFun, _ValueFun, [], Accum) ->
    Accum;
groups_loop(KeyFun, ValueFun, [Elem | Rest], Accum) ->
    Key = KeyFun(Elem),
    Value = ValueFun(Elem),
    Group =
    ...
    groups_loop(KeyFun, ValueFun, Rest, maps:put(Key, Group, Accum)).

%% @private
from_keys_loop([], _Value, Accum) ->
    Accum;
from_keys_loop([Key | Rest], Value, Accum) ->
    from_keys_loop(Rest, Value, maps:put(Key, Value, Accum)).
