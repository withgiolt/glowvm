...
%%
%% ---------------------------------------------------------------------------
%% Vendored verbatim into GlowVM from OTP's stdlib (Apache-2.0) — see
%% THIRD_PARTY_LICENSES. Do not edit; re-copy from
%% `code:lib_dir(stdlib) ++ "/src/io_lib_format.erl"` to update.
%%
%% AtomVM's estdlib has no io_lib_format, but gleam_stdlib:float_to_string/1
%% (behind gleam/float's to_string, and string.inspect of any Float) calls
%% io_lib_format:fwrite_g/1. On AtomVM that is the only function ever reached,
%% and it is a one-liner over erlang:float_to_list/2, which AtomVM implements
%% as a NIF that accepts the `short` option — so this works there as-is.
%%
%% The rest of the file is here because this module also joins the *host* code
%% path and shadows OTP's own. io_lib:format/2 calls io_lib_format:fwrite/2,
%% so a shim exporting only fwrite_g/1 would break every host caller's string
%% formatting. Copying OTP's implementation whole makes the shadowing a no-op
%% on the host by construction.
%% ---------------------------------------------------------------------------
